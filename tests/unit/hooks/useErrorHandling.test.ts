...
describe('useErrorHandling', () => {
  ...
  it('should handle async operations successfully', async () => {
    ...
    await act(async () => {
      operationResult = await result.current.handleAsyncOperation(mockOperation, 'async_test')
    })
  ...
  it('should handle async operation failures', async () => {
    const { result } = renderHook(() => useErrorHandling())
    const mockOperation = vi.fn().mockRejectedValue(new Error('Async operation failed'))

    let operationResult: string | null = 'initial'

    await act(async () => {
      operationResult = await result.current.handleAsyncOperation(mockOperation, 'async_fail')
    })
  ...
  it('should show retry state during operations', async () => {
    ...
    const operationPromise = act(async () => {
      return result.current.retry(slowOperation)
    })

    // Check that retry state is set during operation
