...
test.describe('Order Management Dashboard', () => {
  ...
  test.describe('Dashboard Loading and Display', () => {
    test('should load order dashboard successfully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="dashboard-controls"]')).toBeVisible()
    })
    ...
    test('should display order statistics', async ({ page }) => {
      ...
      if (await statsSection.isVisible()) {
        await expect(statsSection).toContainText(/pending|ready|completed/i)
      }
    })
  ...
  test.describe('Order Display and Management', () => {
    ...
    test('should allow status updates for orders', async ({ page }) => {
      ...
      if (orderCount > 0) {
        ...
        const statusButton = firstOrder.locator('button').filter({ hasText: /ready|complete|pending/i }).first()
        ...
        if (await statusButton.isVisible()) {
          ...
          const updatedButton = firstOrder.locator('button').filter({ hasText: /ready|complete|pending/i }).first()
  ...
  test.describe('Real-time Updates', () => {
    ...
    test('should handle reconnection attempts', async ({ page }) => {
      ...
      const reconnectButton = page.locator('button').filter({ hasText: /reconnect|refresh/i })
...
test.describe('Order Management Workflow Integration', () => {
  ...
  test('should handle complete order lifecycle', async ({ page }) => {
    ...
    if (orderCount > 0) {
      ...
      const statusButtons = testOrder.locator('button').filter({ hasText: /ready|complete|pending/i })
