...
// Test configuration (removed BASE_URL - using baseURL from config)
...
test.describe("Guest Ordering Experience Improvements", () => {
  ...
  test.describe("Funny Name Generation", () => {
    test("should allow guests to generate funny names", async ({ page }) => {
      ...
      const startOrderingButton = page.locator("button").filter({
        hasText: /start|order|menu/i,
      }).first();
      ...
      const guestNameInput = page.locator('input[placeholder*="name" i]')
        .first();
      if (await guestNameInput.isVisible()) {
        ...
        if (await generateButton.count() > 0) {
          ...
          expect(generatedName.toLowerCase()).toMatch(
            /coffee|espresso|latte|brew|bean|roast|grind|cup|java|mocha|cappuccino|macchiato|americano|caffeine/i,
          );
        }
      }
    });
      ...
      if (await guestNameInput.isVisible()) {
        ...
        if (await generateButton.count() > 0) {
          ...
          const regenerateButton = page.locator(
            'button[aria-label*="generate new" i], button[title*="regenerate" i]',
          );
        ...
        if (await guestNameInput.isVisible()) {
          ...
          if (await nextButton.count() > 0) {
            ...
            if (await submitButton.count() > 0) {
              ...
              const proverbElement = page.locator(
                '[data-testid="barista-proverb"], .barista-proverb, [class*="proverb"]',
              );
              ...
              if (await proverbElement.isVisible()) {
                ...
                expect(proverbText?.toLowerCase()).toMatch(
                  /patience|wait|time|perfect|brew|ready/i,
                );
              }
            }
          }
        ...
        if (await guestNameInput.isVisible()) {
          ...
          if (await nextButton.count() > 0) {
            ...
            if (await submitButton.count() > 0) {
              ...
              const waitTimeElement = page.locator(
                '[data-testid*="wait"], [class*="wait"], text=/minutes?/i',
              );
  ...
  test.describe("Guest Information Form Enhancements", () => {
      ...
      if (await guestNameInput.isVisible()) {
        ...
        if (await generateButton.count() > 0) {
          ...
          const helperText = page.locator(
            "text=/fun.*name|given.*you|coffee.*name|click.*enter/i",
          );
    ...
    test("should handle special requests appropriately", async ({ page }) => {
      ...
      const specialRequestInput = page.locator(
        'textarea[placeholder*="special" i], input[placeholder*="request" i]',
      );
      ...
      if (await specialRequestInput.isVisible()) {
        ...
        const charCount = page.locator(
          "text=/\\d+.*character|\\d+.*remaining/i",
        );
        ...
        if (await guestNameInput.isVisible()) {
          ...
          if (await nextButton.count() > 0) {
            ...
            const hasProgressed = currentUrl.includes("menu") ||
              await page.locator('[data-testid*="menu"], [class*="menu"]')
                  .count() > 0;
    ...
    test("should have proper ARIA labels and accessibility attributes", async ({ page }) => {
      ...
      if (await guestNameInput.isVisible()) {
        ...
        if (await regenerateButton.count() > 0) {
          const ariaLabel = await regenerateButton.first().getAttribute(
            "aria-label",
          );
