...
/**
 * E2E tests for configurable telemetry disabled state
 *
 * These tests verify that the application works correctly when configurable telemetry
 * (Vercel and Supabase) is disabled while allowing always-enabled telemetry
 * (Google Analytics and Microsoft Clarity) which are hardcoded in index.html.
 */

test.describe("Application with Configurable Telemetry Disabled", () => {
  ...
  test("should navigate to guest module without telemetry issues", async ({ page }) => {
    ...
    const orderInterface = page.locator(
      '[data-testid="guest-order-form"], .order-form, .guest-module, .menu-section',
    );
    ...
    const hasContent = passwordVisible || interfaceVisible ||
  ...
  test("should allow hardcoded telemetry but block configurable telemetry requests", async ({ page }) => {
    ...
    // Check that configurable telemetry requests are blocked
    // (Vercel Speed Insights, Supabase telemetry endpoints)
    const configurableTelemetryRequests = networkRequests.filter((url) =>
      (url.includes("vercel") && url.includes("speed-insights")) ||
      (url.includes("supabase") && url.includes("telemetry")) ||
      url.includes("vitals")
    );

    // Configurable telemetry should be disabled in test environment
    expect(configurableTelemetryRequests).toHaveLength(0);

    // Hardcoded telemetry (Google Analytics, Microsoft Clarity) is expected
    const hardcodedTelemetryRequests = networkRequests.filter((url) =>
      url.includes("google-analytics.com") ||
      url.includes("googletagmanager.com") ||
      url.includes("clarity.ms")
    );

    // Log the presence of hardcoded telemetry for verification
    if (hardcodedTelemetryRequests.length > 0) {
      console.log(
        `✓ Hardcoded telemetry working as expected: ${hardcodedTelemetryRequests.length} requests`,
      );
    }
