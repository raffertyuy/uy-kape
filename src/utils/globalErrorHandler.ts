...
const defaultConfig: GlobalErrorConfig = {
  enableLogging: true,
  logLevel: 'error',
  enableDevDetails: process.env.NODE_ENV === 'development'
}
...
export const configureGlobalErrorHandler = (config: Partial<GlobalErrorConfig>) => {
...
export const getUserFriendlyMessage = (error: any, category: ErrorCategory): string => {
...
export const getRecoveryStrategy = (errorCategory: ErrorCategory, error?: any): RecoveryStrategy => {
  switch (errorCategory) {
    case 'network':
      if (!getOnlineStatus()) {
        return {
          type: 'manual',
          message: 'Please check your internet connection and try again when you\'re back online.'
        }
      }
      return {
        type: 'retry',
...
export const handleGlobalError = (error: any, context?: string): ErrorDetails => {
