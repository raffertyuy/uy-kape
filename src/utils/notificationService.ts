...
class BrowserNotificationService implements NotificationServiceInterface {
  ...
  async playAudio(type: NotificationOptions['audioType'] = 'new-order'): Promise<void> {
    try {
  ...
  private getFrequencyForType(type?: string): number {
    switch (type) {
      case 'new-order': return 800 // High, attention-getting
      case 'order-ready': return 600 // Medium, pleasant
      case 'order-completed': return 400 // Low, completion
      case 'error': return 300 // Low, concerning
      default: return 600
    }
  }
}
...
export function useNotifications() {
  const [permission, setPermission] = useState<string>(notificationService.permission)
  const [enabled, setEnabled] = useState(notificationService.areNotificationsEnabled())
