...
export const OrderStats = ({
  ...
  if (error) {
    return (
      <div className={cn('bg-red-50 border border-red-200 rounded-lg p-4 dark:bg-red-900/20 dark:border-red-800', className)}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="space-y-2">
                ...
                <div className="h-6 bg-gray-300 rounded w-1/2 dark:bg-gray-600" />
              </div>
            ))}
          </div>
        </div>
  ...
  if (!stats) {
    return (
      <div className={cn('bg-gray-50 border border-gray-200 rounded-lg p-4 dark:bg-gray-800 dark:border-gray-700', className)}>
        <div className="text-center text-gray-500 dark:text-gray-400">
          <p className="text-sm">No statistics available</p>
          {onRefresh && (
            <button
              onClick={onRefresh}
              className="mt-2 text-sm text-coffee-600 hover:text-coffee-700 dark:text-coffee-400 dark:hover:text-coffee-300"
            >
              Load Statistics
            </button>
          )}
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      default:
        return { color: 'text-gray-600 dark:text-gray-400', bg: 'bg-gray-100 dark:bg-gray-900/20' }
    }
  }
  ...
  return (
    <div className={cn('bg-white border border-gray-200 rounded-lg p-4 dark:bg-gray-800 dark:border-gray-700', className)}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        {/* Total Today */}
        <div className="p-3 rounded-lg bg-coffee-100 dark:bg-coffee-900/20">
          <div className="flex items-center justify-between">
            <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Avg Wait Time</p>
