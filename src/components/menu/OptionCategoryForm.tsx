...
interface OptionCategoryFormProps {
  initialData?: OptionCategory
  onSubmit: (_data: CreateOptionCategoryDto | UpdateOptionCategoryDto) => Promise<void>
  onCancel: () => void
  isLoading?: boolean
}

export const OptionCategoryForm: React.FC<OptionCategoryFormProps> = ({
  initialData,
  onSubmit,
  onCancel,
  isLoading = false
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialData) {
        ...
        const updateData: UpdateOptionCategoryDto = {
        ...
        // Create new category
        const createData: CreateOptionCategoryDto = {
          name: formData.name.trim(),
          description: formData.description.trim() || null,
          is_required: formData.is_required,
          display_order: formData.display_order
      ...
      console.error('Error submitting option category form:', error)
      setErrors({
  ...
  const validateForm = (data: typeof formData) => {
    ...
    // Name validation
    if (!data.name.trim()) {
      errors.name = 'Option category name is required.'
    } else if (data.name.trim().length < 2) {
      errors.name = 'Option category name must be at least 2 characters long.'
    } else if (data.name.trim().length > 100) {
      errors.name = 'Option category name must be no more than 100 characters long.'
    }
    ...
    <div className="p-6">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-coffee-800">
          {initialData ? 'Edit Option Category' : 'Add New Option Category'}
        </h2>
        <p className="text-coffee-600 mt-1">
          {initialData 
            ? 'Update the option category details below.'
            : 'Create a new option category for drink customizations.'
          }
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., Number of Shots, Milk Type, Size"
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Optional description of this option category..."
            disabled={isSubmitDisabled}
            maxLength={500}
          />
          {errors.description && (
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-coffee-500">
            When enabled, customers will be required to make a selection from this category.
          </p>
        </div>
