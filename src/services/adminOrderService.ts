...
const handleAdminSupabaseError = (error: any, context?: string): never => {
  console.error(
    `Admin order operation failed${context ? ` (${context})` : ""}:`,
    error,
  );
...
export const adminOrderService = {
  ...
  getAllOrders: async (
    ...
    try {
      ...
      query = query.order("queue_number", {
        ascending: true,
        nullsFirst: false,
      });
  ...
  clearAllPendingOrders: async (): Promise<BulkOperationResult> => {
    try {
      ...
      if (fetchError) {
        handleAdminSupabaseError(fetchError, "clearAllPendingOrders");
      }
  ...
  getQueueManagement: async (): Promise<QueueManagement> => {
    try {
      ...
      const currentPosition = totalOrders > 0
        ? (pendingOrders?.[0]?.queue_number || 1)
        : 0;
