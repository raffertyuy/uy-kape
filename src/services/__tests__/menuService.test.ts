...
describe("menuService", () => {
  ...
  describe("drinkCategoriesService", () => {
    ...
    it("should handle errors when getting categories", async () => {
      ...
      await expect(drinkCategoriesService.getAll()).rejects.toThrow(
        "Database error",
      );
    });
    ...
    it("should handle PGRST116 error when updating non-existent category", async () => {
      const updates = { name: "Updated Coffee" };
      const mockError = {
        code: "PGRST116",
      ...
      await expect(drinkCategoriesService.update("non-existent-id", updates))
        .rejects.toThrow("The result contains 0 rows");
    });
  ...
  describe("drinksService", () => {
    ...
    it("should update an existing drink", async () => {
      const updates = {
        ...
        description: "Updated strong coffee shot",
      };
      ...
      expect(result.name).toBe("Updated Espresso");
      expect(result.description).toBe("Updated strong coffee shot");
    });

    it("should handle PGRST116 error when updating non-existent drink", async () => {
      const updates = { name: "Updated Espresso" };
      const mockError = {
        code: "PGRST116",
        message: "The result contains 0 rows",
      };
  ...
  describe("optionCategoriesService", () => {
    ...
    it("should update an existing option category", async () => {
      const updates = {
        name: "Updated Size",
        description: "Updated drink sizes",
      };
      const updatedOptionCategory = { ...mockOptionCategory, ...updates };
      ...
      const result = await optionCategoriesService.update("1", updates);
      expect(result.name).toBe("Updated Size");
      expect(result.description).toBe("Updated drink sizes");
    });

    it("should handle PGRST116 error when updating non-existent option category", async () => {
      ...
      await expect(optionCategoriesService.update("non-existent-id", updates))
        .rejects.toThrow("The result contains 0 rows");
    });
  });
  ...
  describe("optionValuesService", () => {
    ...
    it("should handle validation errors", async () => {
      ...
      await expect(optionValuesService.create(invalidOptionValue as any))
