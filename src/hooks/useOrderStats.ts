...
import type { OrderStatistics, AdminOrderError, QueueManagement } from '@/types/admin.types'
 ...
 */
export const useOrderStats = (options: UseOrderStatsOptions = {}): UseOrderStatsReturn => {
  const {
    dateRange,
    autoRefresh = true,
...
export const formatStatistics = (stats: OrderStatistics | null) => {
  ...
  return {
    ...
    completionRate: stats.total_completed + stats.total_cancelled > 0
      ? Math.round((stats.total_completed / (stats.total_completed + stats.total_cancelled)) * 100)
      : 0
  }
}
