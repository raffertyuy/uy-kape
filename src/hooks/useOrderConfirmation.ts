...
import type { OrderSubmissionResult, OrderWithDetails, OrderServiceError } from '@/types/order.types'
...
export function useOrderConfirmation(orderId: string | null): UseOrderConfirmationReturn {
  ...
  const fetchOrderData = useCallback(async (orderIdToFetch: string): Promise<void> => {
    try {
      setError(null)
      setIsLoading(true)
      
      // Validate orderId format (basic check)
      if (!orderIdToFetch || orderIdToFetch.length < 10) {
        throw {
          type: 'validation',
      ...
      if (!orderResult) {
        const result: OrderSubmissionResult = {
          ...
          ...(queueStatusHook.queueStatus?.estimatedWaitTime && {
            estimated_wait_time: queueStatusHook.queueStatus.estimatedWaitTime
          })
        }
      ...
      if (err && typeof err === 'object' && 'type' in err) {
        ...
        if (err.message.includes('fetch')) {
          ...
          setError({
            type: 'database',
            message: 'Order not found. It may have been completed or cancelled.'
          })
        } else {
          setError({
            type: 'unknown',
        ...
        setError({
          type: 'unknown',
          message: 'An unexpected error occurred while loading your order confirmation.'
        })
      }
    } finally {
      setIsLoading(false)
